...
#Step 8
#Here we create a new data set where we calculate the mean of each measurement by subject and activity and clean column names
...
column_names
names(sub_act_mean) <- column_names
names(sub_act_mean)

write.table(sub_act_mean, file = "mean_by_subject_activity.txt", sep = ",", row.name = FALSE)
write.table(names(sub_act_mean), sep = ",", row.name = FALSE) 
